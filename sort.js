	...
	for (var i=0; i<10000;i++) {
		...
		//Сортування злиттям
		//рекурсивно ділимо масив на дві частини, поки в кожному підмасиві
		//залишеться один елемент, потім використовуємо злиття(також рекурсивно)
		//порівнюючи при цьому елементи
		//підмасиви зберігаються у стек(max=10 000)
		testArray.sortMerge = function(p,r) {
			...
			function startMerge(p,r) {
				if (p < r) {
					...
					var q = Math.floor((p+r)/2); // середина підмасива
					startMerge(p,q); //рекурсивний виклик фунції
					startMerge(q+1,r);
					merge(p,q,r);
				}
			}
			...
			function merge(p,q,r) {
				...
				var j=0; 
				//записуємо елементи у підмасиви 
				for (var i = p; i<=q; i++) {
					
					B[j] = testArray[i];
					j++;
					// вставка ГРАНИЦЬ(якесь велике число, більше за будьякий 
					// елемет підмасиву), потрібно коректного обміну 
					// елементів підмасиву(сортування при злитті) *
				...
				i=0;
				j=0;
				for (var k = p; k <=r; k++) { //*щоб останній елемент підмасиву 
					//записався в масив використовуємо ГРАНИЦІ
					//тобто виконається перша перевірка, навіть найбільший елемент 
					//підмасиву буде менший за ГРАНИЦЮ і коректно запишеться 
					//в основний масив
					if (B[i]<=C[j]) {
						testArray[k] = B[i];
						i++;
